...
public class Model
{
    ...
    public static final int MINIMAX_TIME = 11;

    // תכונות
    private Board logicBoard;   // לוח משחק לוגי
    private int numMoves = 0; // סופר עד  חמישה מהלכים ואם לא קורה כלום מכריז תיקו
...
//    }
    /**
     * בודק אם המהלך כולל אכילה על הלוח משחק הנוכחי
     *
     * @param move
...
//    }
    /**
     * בודק האם מהלך שנעשה על לוח מסויים כולל אכילה
     *
     * @param board
     * @param move
    ...
    /**
     * עושה את המהלך שהתקבל כפרמטר על לוח מסויים
     *
     * @param board
    ...
    public boolean checkTie(Board board)
    {
        ...
        if(countKings(board, white) == countKings(board, black)
                && countOrdinaryPieces(board, white) == 0
                && countOrdinaryPieces(board, black) == 0)
        {
            if(numMoves == 5)
            {
                numMoves = 0;
    ...
    public boolean checkWin(Board board, PieceColor playerSign)
    {
        ...
        if(playerSign == BLACK_PLAYER && counterBlack > 0 && counterWhite == 0)
        {
            return true;
        }
...
//        return minimaxMove;
        ...
        while(System.currentTimeMillis() < minimaxTime /*&& (minimaxTime - System.currentTimeMillis()) > (System.currentTimeMillis() - startTime)*/)
        {
            minimaxMove = minimax(logicBoard, current, true, 0, maxDepth++, Integer.MIN_VALUE, Integer.MAX_VALUE);
     ...
     */
    public Move minimax(Board board, PieceColor current, boolean isMax, int depth, int maxDepth, int alpha, int beta)
    {
        ...
        if(isMax)
        {
            ...
            for(int i = 0; i < possibleMoves.size(); i++)
            {
                ...
                Move minimaxMove = minimax(newBoard, current, false, depth + 1, maxDepth, alpha, beta);
        ...
        else
        {
            ...
            for(int i = 0; i < possibleMoves.size(); i++)
            {
                ...
                Board newBoard = new Board(board);
                makeMove(newBoard, move);
                Move minimaxMove = minimax(newBoard, current, true, depth + 1, maxDepth, alpha, beta);
                minimaxMove.setMove(move);
                if(minimaxMove.isDepthLegit()
                        && (minimaxMove.getGrade() < bestMove.getGrade()
                        || (minimaxMove.getIsEat() && !bestMove.getIsEat())
