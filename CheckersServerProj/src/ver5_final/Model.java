...
public class Model
{
    ...
    private void testingSetup()
    {
        ...
        ArrayList<Piece> list = new ArrayList(Arrays.asList(new Piece(WHITE_PLAYER, new Location(4, 3), true),
                ...
                new Piece(BLACK_PLAYER, new Location(2, 3), true),
    ...
    /**
     * Checks if specific player can't move (to know if switch turn or not)
     *
     * @param player
     * @return true if player can't move, false else
     */
    public boolean playerCantMove(PieceColor player)
    {
        for(int i = 0; i < ROWS; i++)
        {
            for(int j = 0; j < COLS; j++)
            {
                ...
                Piece p = pieceInLoc(loc);
                if(p != null)
                {
                    ArrayList<Move> moves = simplePossibleMovesForPiece(logicBoard, loc);
                    if(!moves.isEmpty())
                    {
                        return false;
                    }
...
//    }
    ...
    public boolean checkWin(Board board, PieceColor playerSign)
    {
        ...
        if(playerCantMove(getOpponent(playerSign)))
        {
            return true;
        }
     ...
     * @return minimax move
     */
    public MinimaxMove getAiMove(PieceColor player)
    {
        System.out.println("Model getAiMove() using MINIMAX");

        MinimaxMove bestMove = getBestMoveUsingMinimax(player);

        return bestMove;
    }

    // ============================ for minimax ===========================
    private MinimaxMove getBestMoveUsingMinimax(PieceColor player)
    {
        int maxDepth = 1;
        MinimaxMove minimaxMove = new MinimaxMove();
        MinimaxMove lastMove = new MinimaxMove();
        long minimaxTime = System.currentTimeMillis() + (MINIMAX_TIME * 1000);
        long iterateTime = 0;
        while(System.currentTimeMillis() < minimaxTime
                /* אם הזמן שנותר גדול או שווה לזמן שלקחה האיטרציה האחרונה פלוס חריגה*/
                && minimaxTime - System.currentTimeMillis() >= ((iterateTime * 2) + 1000))
        {
            ...
            System.out.println("iterate time for depth " + maxDepth + ": " + iterateTime + "sec");
            maxDepth++;
...
//            boolean currentOneMoveKing = oneMoveKingCurrentPlayer(player, allMoves),
//                    opponentOneMoveKing = oneMoveKingForOpponent(opponent, opponentMoves, allMoves);
...
//            }
            ...
            boolean opponentOneMoveEaten = ordinaryCouldBeEatenNextMove(board, opponentMoves),
                    ...
                    currentMultEating = hasMultEating(allMoves),
                    opponentMultEating = hasMultEating(opponentMoves);
            if(opponentOneMoveEaten && !opponentKingOneMoveEaten && !opponentMultEating)
            {
                evalScore -= 1;
            }
            ...
            if(currentMultEating)
            {
                evalScore += 3;
            }

            if(opponentMultEating)
            {
                evalScore -= 3;
            }
        }
    ...
    public int evalWithPrints(Board board, PieceColor player)
    {
        ...
        if(checkWin(board, opponent))
        {
            System.out.println("loose for " + player);
            return -100;
        }
...
//            {
//                evalScore += 2;
//                System.out.println("one move king for player! \ngrade now: " + evalScore);
//            }
//
//            if(opponentOneMoveKing)
//            {
//                evalScore -= 2;
//                System.out.println("one move king for opponent next turn! \ngrade now: " + evalScore);
//            }
            if(oneMoveKingCurrentPlayer(player, allMoves))
            {
                System.out.println("OMK exists!\ngrade now: " + evalScore);
                evalScore += 1;
            }

            sub = subPiecesOnFirstLine(board, player);
            evalScore += sub;
            System.out.println("sub pieces on first line: " + sub + "\ngrade now: " + evalScore);
            ...
            {
                evalScore += 3;
                System.out.println("you have mult eating!\ngrade now: " + evalScore);
            }
...
//        }
//    }
    // Returns how many pieces of player can make king next turn
    private boolean oneMoveKingCurrentPlayer(PieceColor player, ArrayList<Move> allMoves)
    {
        for(Move move : allMoves)
        {
            Piece piece = move.getPiece();
            ...
            if(player == WHITE_PLAYER && !piece.isKing()
                    && piece.getPieceColor() == WHITE_PLAYER && rowDest == 0)
            {
                return true;
            }

            if(player == BLACK_PLAYER && !piece.isKing()
                    && piece.getPieceColor() == BLACK_PLAYER
                    && rowDest == ROWS - 1)
            {
                return true;
            }
    ...
    // Returns how many pieces of opponent can make king next turn
    private boolean oneMoveKingForOpponent(PieceColor player, ArrayList<Move> opponentMoves,
            ArrayList<Move> currentMoves)
    {
        ArrayList<Move> onlyDestZero = new ArrayList<>();
        for(Move move : opponentMoves)
        {
            if(move.getDestination().getRow() == 0)
            {
                onlyDestZero.add(new Move(move));
            }
        }

        if(onlyDestZero.isEmpty())
        {
            return false;
        }

        for(Move move : onlyDestZero)
        {
            Piece piece = move.getPiece();
            int rowDest = move.getDestination().getRow();
                    ...
                    && piece.getPieceColor() == WHITE_PLAYER && rowDest == 0)
            {
                ...
                {
                    Move current = currentMoves.get(i);
                    if(current.isEat())
                    {
                        if(current.getEnemyLoc().isEqual(move.getSource()))
                        {
                            onlyDestZero.remove(move);
                        }
                    }
                }
            }
...
//        System.out.println("list before setBurnedLocForAllMoves:");
//        for(Move move : allMoves)
//        {
//            System.out.println(move);
//        }
        allMoves = setBurnedLocForGAPM(allMoves);
        //ArrayList<Move> updatedMoves = setBurnedLocForAllMoves(allMoves, playerSign);
//        System.out.println("last GAPM list:");
//        for(Move move : allMoves)
//        {
//            System.out.println(move);
//        }
...
//        }
    ...
    // Checks if in list of moves we has move of
    // player that can make double eating
    private boolean hasMultEating(ArrayList<Move> allMoves)
    {
        ArrayList<Move> allDoubles = new ArrayList<>();
        for(Move move : allMoves)
        {
            if(move.hasNext())
            {
