...
public class DB
{
    ...
    // Make string from resultSet rs for allGamesPlayed func
    private static String toStringAllGames(ResultSet rs) throws SQLException
    {
        String allGames = "opponent \t winner \t dateTime \n";
        allGames += "-------- \t ------ \t --------- \n";
        while(rs.next())
        {
            String opponent = rs.getNString("opponent");
            String winner = rs.getNString("winner");
            String date = rs.getNString("datetime");
            System.out.println("opponent: " + opponent + ", winner: " + winner + ", dateTime: " + date);

            if(!opponent.startsWith("GUEST"))
            {
                allGames += "  " + opponent + "\t\t";
            }
            else
    ...
    public static String allGamesPlayed(String un) throws SQLException
    {
        ...
        String allGames = toStringAllGames(rs1) + "\n";

        ResultSet rs2 = runQuery(sql2);
        String allGames2 = toStringAllGames(rs2) + "\n\n";

        allGames += allGames2;
        //allGames += allGames2.substring(allGames2.lastIndexOf('-') + 2);
     ...
     * @throws SQLException
     */
    public static String deleteUser(String un)
    {
        ...
        String sql3 = "DELETE * FROM Games Where (un1='" + un + "' OR un2= '"
                + un + "') AND (un2='AI' OR un1 LIKE 'GUEST*' OR"
                + " un2 LIKE 'GUEST*);";
        try
        {
            runUpdate(sql1);
     ...
     *
     * @param un
     * @return String of all games played last week
     */
    public static String gamesPlayedLastWeek(String un)
    {
        String sql = "DELETE * FROM unfinishedGames WHERE dateTime='" + un + "';";
        try
        {
            ResultSet rs = runQuery(sql);
            System.out.println("deleteUnfinshedBoard: " + sql);
            return toStringResultSet(rs);
        }
        catch(SQLException ex)
        {
            ex.printStackTrace();
     ...
     * @return
     */
    public static String gamesVsAI(String un)
    {
...
//        saveBoardToDB("nofar", board, "nofar");
        saveBoardToDB("a", board, "AI");
//        saveBoardToDB("ariel", board, "AI");
        saveBoardToDB("a", board, "a");
//        saveBoardToDB("nofar", board, "AI");
//        saveBoardToDB("ariel", board, "ariel");

        //String allGames = allGamesPlayed("a");
