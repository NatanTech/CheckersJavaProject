...
public class Server
{
     ...
     *
     * @param player1
     * @param player2 The function enter 2 player1 and player2 to new game and
     * run the game
     */
    ...
    /**
     * אם יש לשחקן משחקים שהוא לא סיים אז שולחים לו אותם לבחור אחד מהם או להתחיל
     * משחק חדש
     *
     * @param player
     */
    ...
    private void processCmdFromClient(Message msg, PlayerNet player) throws SQLException
    {
        ...
        if(subject.equals(Constants.SAVE_AND_EXIT))
        {
            ...
            log("Player " + un + " saved game and disconnected!");
        }

        if(subject.equals(Constants.ALL_GAMES_PLAYED))
        {
            String res = DB.allGamesPlayed(un);
            Message allGamesMsg = new Message(Constants.ALL_GAMES_PLAYED);
            allGamesMsg.setStats(res);
            cmd.writeMessage(allGamesMsg);
        }

        if(subject.equals(Constants.GAMES_PLAYED_LAST_WEEK))
        {
            String res = DB.gamesPlayedLastWeek(un);
            Message allGamesMsg = new Message(Constants.GAMES_PLAYED_LAST_WEEK);
            allGamesMsg.setStats(res);
            cmd.writeMessage(allGamesMsg);
        }
        ...
        if(subject.equals(Constants.TOP5))
        {
            ...
            top5Msg.setStats(top5);
            cmd.writeMessage(top5Msg);
        }

        if(subject.equals(Constants.DELETE_USER))
        {
            String res = DB.deleteUser(un);
            if(res.equals("delete complete!"))
            {
                cmd.writeMessage(new Message(Constants.DELETE_USER));
            }
            else
            {
                cmd.writeMessage(new Message(Constants.DELETE_USER_FAILED));
            }
        }
     ...
     *
     * @param playerID
     * @return the current player turn in game of specific player
     */
    ...
    // שולח ליריב שהשחקן שמולו התנתק ואם אחד מהם רשום שומר את התוצאה
     ...
     *
     * @param ID
     * @return true if specific player is already online, false else
     */
