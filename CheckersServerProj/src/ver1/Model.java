...
public class Model implements Serializable
{
    ...
    {
        this.logicBoard = model.logicBoard;
        ...
        for(int row = 0; row < ROWS; row++)
        {
            ...
            for(int col = 0; col < COLS; col++)
            {
                ...
                if(isBlackTile)
                {
                    if(row >= 0 && row <= 2)
                    {
                        Piece p = new Piece(Player.black, loc, false);
                        logicBoard.setPiece(loc, p);
                    }
                    
                    if(row >= 5 && row <= 7)
                    {
                        Piece p = new Piece(Player.white, loc, false);
                        logicBoard.setPiece(loc, p);
                    }
    ...
    {
        ArrayList<Piece> list = new ArrayList(Arrays.asList(new Piece(Player.white, new Location(4, 3), true),
                new Piece(Player.white, new Location(4, 7), false),
                new Piece(Player.black, new Location(2, 3), true),
                new Piece(Player.black, new Location(2, 7), false)));
    ...
    public Location getTheEatChoosed(Board board, Move move)
    {
        ...
        if(player == Player.white || (player == Player.black && piece.isKing()))
        {
        ...
        if(player == Player.black || (player == Player.white && piece.isKing()))
        {
            Location newSource1 = new Location(source.getRow() + 2, source.getCol() + 2);
            Location newSource2 = new Location(source.getRow() + 2, source.getCol() - 2);
    ...
    public void makeMove(Board board, Move move)
    {
        ...
        if((player == Player.white && destination.getRow() == 0)
                || player == Player.black && destination.getRow() == ROWS - 1)
        {
    ...
    public boolean checkTie(Board board)
    {
        ...
        return countKings(board, white) == countKings(board, black)
     ...
     * else
     */
    public boolean checkWin(Player playerSign)
    {
     ...
     * else
     */
    public boolean checkWin(Board board, Player playerSign)
    {
        int counterWhite = 0;
        int counterBlack = 0;
        Player opponent = getOpponent(playerSign);
        ...
        if(playerSign == Player.white && counterBlack == 0)
        {
            return true;
        }

        if(playerSign == Player.black && counterWhite == 0)
        {
            return true;
        }
...
//
//        Move bestMove = getBestMoveUsingMinimax();
//
//        return bestMove;
//    }
//
//    // ============================ for minimax ===========================
//    private Move getBestMoveUsingMinimax()
//    {
//        Move minimaxMove = minimax(logicBoard, true, 0, Integer.MIN_VALUE, Integer.MAX_VALUE);
...
//    }
     ...
     */
//    public Move minimax(Board board, boolean isMax, int depth, int alpha, int beta)
//    {
...
//                Move minimaxMove = minimax(newBoard, false, depth + 1, alpha, beta);
...
//                Board newBoard = new Board(board);
//                makeMove(newBoard, move);
//                Move minimaxMove = minimax(newBoard, true, depth + 1, alpha, beta);
//                minimaxMove.setMove(move);
//                if(minimaxMove.isDepthLegit()
//                        && (minimaxMove.getGrade() < bestMove.getGrade()
//                        || (minimaxMove.getIsEat() && !bestMove.getIsEat())
...
//    }
     ...
     */
    private int oneMoveKing(Board board, Player player)
    {
        int counter = 0;
        for(int i = 1; i < ROWS; i += 5)
        {
            for(int j = 0; j < COLS; j++)
            {
                ...
                if(piece != null)
                {
                    if(j + 1 < 8)
                    {
                        if((player == Player.white && piece.getPieceColor() == Player.white
                                && i == 1 && board.getPiece(0, j + 1) == null)
                                || (player == Player.black && piece.getPieceColor() == Player.black
     ...
     */
    private int ordinaryCouldBeEatenNextMove(Board board, Player player)
    {
        int counter = 0;
     ...
     * of the opponent
     */
    private int kingsCouldBeEatenNextMove(Board board, Player player)
    {
        int counter = 0;
        ArrayList<Location> existsEnemyLocs = new ArrayList<>();
        Player opponent = getOpponent(player);
     ...
     */
    private int countOrdinaryPieces(Board board, Player player)
    {
        int counter = 0;
     ...
     * specific board
     */
    private int countOrdinaryTroops(Board board, Player player)
    {
        int counter = 0;
        for(int i = 0; i < ROWS; i++)
        {
     ...
     */
    private int countKings(Board board, Player player)
    {
        int counter = 0;
        {
            for(int i = 0; i < ROWS; i++)
     ...
     */
    private ArrayList<Move> getAllPossibleMoves(Board board, Player playerSign)
    {
        ...
        for(int row = 0; row < ROWS; row++)
        {
            for(int col = 0; col < COLS; col++)
            {
                ...
                if(p != null)
                {
                    if(playerSign == Player.white && p.isWhite())
                    {
                        posMoves = possibleMovesForPlayer(board, loc);
                    }

                    if(playerSign == Player.black && !p.isWhite())
                    {
                        posMoves = possibleMovesForPlayer(board, loc);
                    }
    ...
    {
        return checkWin(board, Player.white) || checkWin(board, Player.black)
                || checkTie(board);
    }
    ...
    public ArrayList<Move> possibleMovesForPlayer(Board board, Location loc)
    {
        ...
        for(int i = 0; i < posEatMoves.size(); i++)
        {
            legalMoves.add(posEatMoves.get(i));
        }
        return legalMoves;
    }
     ...
     */
    public ArrayList<Location> getAllEnemyLocs(Player playerSign)
    {
     ...
     * board
     */
    public ArrayList<Location> getAllEnemyLocs(Board board, Player playerSign)
    {
     ...
     * board
     */
    public ArrayList<Location> getAllCurrentLocs(Player currentPlayer)
    {
        ...
        for(int i = 0; i < ROWS; i++)
        {
            for(int j = 0; j < COLS; j++)
            {
                ...
                if(p != null)
                {
                    if(currentPlayer == Player.white && p.isWhite())
                    {
                        currentLoc.add(loc);
                    }

                    if(currentPlayer == Player.black && !p.isWhite())
                    {
                        currentLoc.add(loc);
                    }
                }
